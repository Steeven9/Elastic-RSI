...
function Overview() {
  ...
  useEffect(async () => {
    ...
    const result = {
      x: {
        start: new Date(
          response.aggregations.by_minute.buckets[0].key_as_string
        ),
        end: new Date(
          response.aggregations.by_minute.buckets[
            numberMinutes - 1
          ].key_as_string
        ),
      },
      y: response.aggregations.by_minute.buckets.map(
  ...
  function isDrillingDownFromDaysToHours(currentsource, indexRange) {
    return (
      currentsource.currentTimeLevel === "days" &&
      currentsource.timeLevelIndices.base.days.start <= indexRange.start &&
      indexRange.end <= currentsource.timeLevelIndices.base.days.end
    );
  }

  function isDrillingDownFromHoursToMinutes(currentsource, indexRange) {
    return (
      currentsource.currentTimeLevel === "hours" &&
      currentsource.timeLevelIndices.base.hours.start <= indexRange.start &&
      indexRange.end <= currentsource.timeLevelIndices.base.hours.end
    );
  }
  ...
  function isDrillingUpFromHoursToDays(currentsource, indexRange) {
    return (
      currentsource.currentTimeLevel === "hours" &&
      typeof indexRange.start === "undefined" &&
      typeof indexRange.end === "undefined"
    );
  }

  function isDrillingUpFromMinutesToHours(currentsource, indexRange) {
    return (
      currentsource.currentTimeLevel === "minutes" &&
      indexRange.start === currentsource.timeLevelIndices.days.start &&
      indexRange.end === currentsource.timeLevelIndices.days.end
    );
  }
  ...
  const onDataZoom = useCallback((params) => {
    ...
    if (isDrillingDownFromDaysToHours(sourceReference.current, indexRange)) {
      ...
      drillUpToHours(baseDataReference.current, dayRange).then(
        (dataPerHour) => {
          setSource((source) => {
            let newSource = {
              ...source,
      ...
      isDrillingDownFromHoursToMinutes(sourceReference.current, indexRange)
    ) {
      const start = new Date(
      ...
      selectMinutes(baseDataReference.current, hourRange).then(
        (dataPerMinute) => {
          setSource((source) => {
            let newSource = {
              ...source,
      ...
      isDrillingUpFromHoursToDays(sourceReference.current, indexRange)
    ) {
      ...
      isDrillingUpFromMinutesToHours(sourceReference.current, indexRange)
    ) {
      const dayRange = {
...
function getDayDifference(dayRange) {
  return (
    (dayRange.end.getTime() - dayRange.start.getTime()) / (24 * 60 * 60 * 1000)
  );
}

function getHourDifference(hourRange) {
  return (
    (hourRange.end.getTime() - hourRange.start.getTime()) / (60 * 60 * 1000)
  );
}
...
async function selectMinutes(baseData, hourRange) {
  const startTime = new Date(
    Math.max(baseData.x.start.getTime(), hourRange.start.getTime())
  );
  const startIndex =
    (startTime.getTime() - baseData.x.start.getTime()) / (60 * 1000);
  const endTime = new Date(
    Math.min(hourRange.end.getTime() + 60 * 60 * 1000, baseData.x.end.getTime())
  );
  const endIndex =
    startIndex + (endTime.getTime() - startTime.getTime()) / (60 * 1000);
  const numberMinutes =
    (endTime.getTime() -
      endTime.getSeconds() * 1000 -
      (startTime.getTime() - startTime.getSeconds() * 1000)) /
    (60 * 1000);
  ...
  return {
    x: Array(y.length)
      .fill(0)
      .map((_, i) => new Date(startTime.getTime() + i * 60 * 1000)),
    y: y,
  };
}
...
async function drillUpToHours(baseData, dayRange) {
  const startTime = new Date(
    Math.max(baseData.x.start.getTime(), dayRange.start.getTime())
  );
  const startIndex =
    (startTime.getTime() - baseData.x.start.getTime()) / (60 * 1000);
  const endTime = new Date(
    Math.min(
      dayRange.end.getTime() + 24 * 60 * 60 * 1000,
      baseData.x.end.getTime()
    )
  );
  const endIndex =
    startIndex + (endTime.getTime() - startTime.getTime()) / (60 * 1000);
  ...
  return {
    x: Array(y.length)
      .fill(0)
      .map((_, i) => new Date(startTime.getTime() + i * 60 * 60 * 1000)),
    y: y,
  };
}
...
async function drillUpToDays(baseData) {
  ...
  let minutesIndex =
    (baseData.x.start.getTime() - dayRange.start.getTime()) / (60 * 1000);
  ...
  return {
    x: Array(y.length)
      .fill(0)
      .map(
        (_, i) => new Date(dayRange.start.getTime() + i * 24 * 60 * 60 * 1000)
      ),
    y: y,
  };
}
